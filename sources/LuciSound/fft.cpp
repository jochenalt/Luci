         ...
         float *RealIn, float *ImagIn, float *RealOut, float *ImagOut)
{
   ...
   for (BlockSize = 2; BlockSize <= NumSamples; BlockSize <<= 1) {
      ...
      float sm2 = sinFast(-2 * delta_angle);
      float sm1 = sinFast(-delta_angle);
      float cm2 = cosFast(-2 * delta_angle);
      float cm1 = cosFast(-delta_angle);
      float w = 2 * cm1;
...
void RealFFT(int NumSamples, float *RealIn, float *RealOut, float *ImagOut)
{
   ...
   float wtemp = float (sinFast(0.5 * theta));

   float wpr = -2.0 * wtemp * wtemp;
   float wpi = float (sinFast(theta));
   float wr = 1.0 + wpr;
   float wi = wpi;
...
void WindowFunc(int whichFunction, int NumSamples, float *in)
{
   ...
   if (whichFunction == 2) {
      // Hamming
      for (i = 0; i < NumSamples; i++)
         in[i] *= 0.54 - 0.46 * cosFast(2 * M_PI * i / (NumSamples - 1));
   }

   if (whichFunction == 3) {
      // Hanning
      for (i = 0; i < NumSamples; i++)
         in[i] *= 0.50 - 0.50 * cosFast(2 * M_PI * i / (NumSamples - 1));
   }
}
...
void fft::powerSpectrum(int start, int half, float *data, int windowSize,float *magnitude) {
	int i;
   int windowFunc = 3;
   ...
   if (false) {
	   ...
	   for (i = 0; i < half; i++) {
		   magnitude[i] = 2.0*sqrtf(out_real[i]*out_real[i] + out_img[i]*out_img[i]);
	   }
   }
   else {
	   PowerSpectrum(windowSize, in_real, out_real);

	   for (i = 0; i < half; i++) {
		   magnitude[i] = (out_real[i]);
	   }
   }
   delete[]in_real;
   delete[]out_real;
